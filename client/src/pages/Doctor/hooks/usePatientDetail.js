  ...
  const fetchPatientDetail = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        apiClient.get(
          `/api/v1/doctor/current-exam/${patientId}${
            appointmentId ? `?appointmentId=${appointmentId}` : ""
          }`
        ),
        // 2. Lấy thông tin phác đồ ARV hiện tại
        apiClient.get(`/api/v1/doctor/current-arv/${patientId}`),
        // 3. Lấy kết quả xét nghiệm gần nhất
        apiClient.get(`/api/v1/doctor/latest-tests/${patientId}`),
        // 4. Lấy lịch sử khám
        apiClient.get(`/api/v1/doctor/exam-history/${patientId}`),
      ]);
