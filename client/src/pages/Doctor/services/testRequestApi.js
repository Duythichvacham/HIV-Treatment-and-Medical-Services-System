...
export const testRequestApi = {
  ...
  getById: async (requestId) => {
    const url = `${import.meta.env.VITE_API_PREFIX}/doctors/test-request-details/${requestId}`;
  ...
  getHistory: async (patientId, page = 1, limit = 10) => {
    const url = `${import.meta.env.VITE_API_PREFIX}/doctors/test-requests/${patientId}`;
  ...
  getTestTypes: async () => {
    try {
      const response = await api.get(`${import.meta.env.VITE_API_PREFIX}/doctors/test-types`);
      return response.data;
    } catch (error) {
  ...
  createTestRequest: async (appointmentId, testRequestData) => {
    try {
      ...
      const response = await api.post(
        `${import.meta.env.VITE_API_PREFIX}/doctors/independent-test-requests`,
        payload
      );
      return response.data;
    } catch (error) {
  ...
  getTestRequestsByPatient: async (patientId) => {
    try {
      const response = await api.get(
        `${import.meta.env.VITE_API_PREFIX}/doctors/test-requests/${patientId}`
      );
      return response.data;
    } catch (error) {
  ...
  getTestRequestDetails: async (requestId) => {
    try {
      ...
      const response = await api.get(
        `${import.meta.env.VITE_API_PREFIX}/doctors/test-request-details/${requestId}`
      );
      return response.data;
    } catch (error) {
...
export const prescriptionApi = {
  ...
  getARVRegimens: async () => {
    const response = await api.get(`${import.meta.env.VITE_API_PREFIX}/arv-regimens/`);
    return response.data;
  },

  // Get drugs for specific ARV regimen
  getRegimenDrugs: async (regimenId) => {
    const response = await api.get(`${import.meta.env.VITE_API_PREFIX}/arv-regimens/${regimenId}`);
    return response.data;
  },
  ...
  //   const url = `${import.meta.env.VITE_API_PREFIX}/doctors/prescriptions/${prescriptionId}`;
  ...
  getByAppointment: async (appointmentId) => {
    const url = `${import.meta.env.VITE_API_PREFIX}/doctors/prescriptionDetails`;
