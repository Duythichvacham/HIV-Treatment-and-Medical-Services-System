...
export const examApi = {
  ...
  //   const url = `${import.meta.env.VITE_API_PREFIX}/clinical/clinical-exams/${appointmentId}`;
  ...
  getSavedExamData: async (appointmentId) => {
    try {
      console.log("[examApi.getSavedExamData] API URLs:", {
        clinicalURL: `${import.meta.env.VITE_API_PREFIX}/clinical/clinical-exams/${appointmentId}`,
        prescriptionURL: `${import.meta.env.VITE_API_PREFIX}/prescriptions/${appointmentId}`,
      });

      // Load both clinical exam and prescription data
      const [clinicalResponse, prescriptionResponse] = await Promise.all([
        api.get(`${import.meta.env.VITE_API_PREFIX}/clinical/clinical-exams/${appointmentId}`),
        api.get(`${import.meta.env.VITE_API_PREFIX}/prescriptions/${appointmentId}`),
      ]);
  ...
  save: async (appointmentId, examData, patientData = null) => {
    ...
    try {
      ...
      const clinicalResponse = await api.post(
        `${import.meta.env.VITE_API_PREFIX}/clinical/clinical-exams`,
        clinicalExamData
      );
      ...
      if (examData.prescription) {
        ...
        prescriptionResponse = await api.post(
          `${import.meta.env.VITE_API_PREFIX}/prescriptions/`,
          prescriptionData
        );
      }
  ...
  getById: async (examId) => {
    const url = `${import.meta.env.VITE_API_PREFIX}/doctors/exam-detail/${examId}`;
    const response = await api.get(url);
    return response.data;
  },
