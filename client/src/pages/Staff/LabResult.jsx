  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50 p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl p-8 border">
        <Link to="/lab-staff" className="inline-block bg-green-50 text-green-700 border border-green-200 rounded-full font-bold px-6 py-2 mb-6 hover:bg-green-100 transition">← Quay lại Dashboard</Link>
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Kết quả xét nghiệm</h1>

        {/* Thông tin Bệnh nhân */}
        <div className="mb-8">
          <h2 className="text-xl font-semibold mb-3 text-gray-800">Thông tin Bệnh nhân</h2>
          <div className="grid grid-cols-2 gap-4 text-base text-gray-700 bg-gray-50 rounded-xl p-4 border">
        ...
        <div>
          <h2 className="text-xl font-semibold mb-3 text-gray-800">Chi tiết Kết quả</h2>
              ...
              <tbody>
                {latestResults.map((r, idx) => (
                  <tr key={r.result_id} className="odd:bg-white even:bg-gray-50">
                    ...
                    <td className="border px-4 py-2 font-bold text-gray-900">{r.result_value || '-'}</td>
          ...
          {note.notes && (
            <div className="mt-6 text-base text-gray-700 bg-gray-50 rounded-lg p-4 border"><b>Ghi chú:</b> <span className="text-red-600">{note.notes}</span></div>
          )}
        </div>
      </div>
    </div>
