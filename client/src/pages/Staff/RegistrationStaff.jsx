...
import { Calendar, DollarSign, FileText, Clock, User, Stethoscope, TestTube, Receipt, Search } from 'lucide-react';
...
const RegistrationStaff = () => {
  ...
  const handleProcessPayment = (requestIndex) => {
    ...
    setTestRequests(prev => prev.filter((_, index) => index !== requestIndex));
    setStats(prev => ({
      ...prev,
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white rounded-lg shadow-sm border p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">TestRequests status = 'requested'</p>
                    </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">Tổng tiền đã thu</p>
                    </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-600">Số đơn đã xử lý</p>
                      <p className="text-3xl font-bold text-blue-600">{stats.processedToday}</p>
                      <p className="text-xs text-gray-500 mt-1">Trong ngày hôm nay</p>
                    </div>
                  ...
                  {testRequests.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {testRequests.map((request, index) => (
                        <div key={index} className="bg-white border border-l-4 border-l-orange-500 rounded-lg p-6 shadow-sm">
                          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 items-start">
                            ...
                            <div className="space-y-2">
                              <h4 className="font-medium text-gray-900">Dịch vụ xét nghiệm:</h4>
              ...
              <div className="bg-white rounded-lg shadow-sm border">
                <div className="p-6 border-b">
                  ...
                  <div className="mt-4 flex items-center gap-4 text-sm">
                    ...
                    <span className="font-semibold text-green-600">{formatCurrency(2450000)}</span>
                    <span className="text-gray-600">•</span>
                    <span className="text-gray-600">{completedRequests.length} đơn đã thanh toán</span>
                  </div>
                </div>
                <div className="p-6">
                  {filteredCompletedRequests.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p>{searchTerm ? 'Không tìm thấy kết quả phù hợp' : 'Chưa có đơn nào được thanh toán hôm nay'}</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {filteredCompletedRequests.map((request, index) => (
                        <div key={index} className="bg-white border border-l-4 border-l-green-500 rounded-lg p-6 shadow-sm">
                          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 items-start">
                            ...
                            <div className="space-y-2">
                              <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                                <div className="text-sm text-green-800">Tổng tiền:</div>
                                <div className="text-lg font-bold text-green-600">
                                  {formatCurrency(request.totalAmount)}
                                </div>
                              </div>
                            ...
                            <div className="space-y-2">
                              <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                                <div className="text-sm text-blue-800">Thanh toán:</div>
                                <div className="font-medium text-blue-900">{request.paymentMethod}</div>
                              </div>
                            </div>
                            ...
                            <div className="space-y-2">
                              <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
                                ...
                                <div className="text-sm text-gray-600 mt-1">STT: {request.queueNumber}</div>
                              </div>
