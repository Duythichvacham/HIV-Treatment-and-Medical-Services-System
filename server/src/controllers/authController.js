    ...
    const token = jwt.sign(
      {
        ...
        ...(user.patient_id && {
          ...
          patient_name: user?.patient_name || null,
        }),
      },
...
exports.registerPatient = async (req, res) => {
  const { username, password, fullName, dob, gender, email, phone, address } =
    req.body;
...
exports.changePassword = async (req, res, next) => {
  try {
    ...
    console.log(
      "Check verifiedEmails (changePassword):",
      email,
    ...
    res
      .status(400)
      .json({ message: err.message || "Lỗi server khi đổi mật khẩu." });
  }
};
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    // 1. Kiểm tra xác thực OTP
    console.log(
      "Check verifiedEmails (resetPassword):",
      email,
      verifiedEmails[email]
    );
    if (!verifiedEmails[email] || Date.now() > verifiedEmails[email]) {
    ...
    res
      .status(400)
      .json({ message: err.message || "Lỗi server khi đặt lại mật khẩu." });
  }
};
