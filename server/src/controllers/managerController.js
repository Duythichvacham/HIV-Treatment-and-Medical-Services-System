...
const setActive = async (req, res) => {
 ...
 if (typeof is_active !== "boolean") {
    ...
    const updated = await serviceService.setActiveService(parseInt(serviceId), is_active);
  if (!updated) {
    ...
    return res.status(200).json({
      success: true,
      message: is_active ? "Mở lại dịch vụ thành công" : "Ẩn dịch vụ thành công"
    });
   
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Lỗi khi cập nhật trạng thái cho service ! " + error.message });
  }
};
...
const createSlot = async (req, res) => {
  try {
    ...
    const checkTime = await serviceService.checkTimeService(start_time, end_time);
    ...
    const result = await serviceService.createSlotDB(start_time, end_time);
